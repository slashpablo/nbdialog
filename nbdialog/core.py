...
class Provider(Protocol):
    ...
    def complete(self, messages: list[dict], tools: list = None) -> Turn: ...
...
def get_provider() -> Provider:
    ...
    if _provider is None:
        raise RuntimeError("Call nbdialog.set_provider(OpenAIProvider()) first.")
    return _provider
...
# %% ../nbs/00_core.ipynb #run-completion-code
def run_completion(provider: Provider, messages: list[dict],
                   tools: list[Tool] = None, trace: Trace = None,
                   max_tool_steps: int = 8) -> Turn:
    ...
    for _ in range(max_tool_steps):
        ...
        turn = provider.complete(msgs, tools=tools or None)
        dt = time.perf_counter() - t0
        if trace is not None:
            trace.add_model_turn(turn.text,
                                 [{"name": tc.name, "args": tc.args} for tc in turn.tool_calls],
                                 turn.usage, dt)
        if not turn.tool_calls: return turn
        ...
        for tc in turn.tool_calls:
            t0 = time.perf_counter()
            out = dispatch[tc.name](**tc.args)
            dt = time.perf_counter() - t0
            if trace is not None:
                trace.add_tool_turn(tc.name, tc.args, str(out), dt)
            msgs.append({"role": "tool", "tool_call_id": tc.id, "content": str(out)})
    raise RuntimeError(f"Tool loop did not converge in {max_tool_steps} steps")
...
def prompt(line, cell):
    ...
    turn = run_completion(get_provider(), messages, tools=get_tools() or None, trace=trace)
    if trace is not None: display(trace)
    text = turn.text or "(no content)"
    if turn.finish_reason == "length":
        text = "**Response truncated by max_completion_tokens.**\n\n" + text
